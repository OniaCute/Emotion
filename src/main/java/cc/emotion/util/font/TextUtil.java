...
public class TextUtil implements Wrapper {
	...
	public static float getCustomWidth(String s) {
		return isCustomFont() ? FontRenderers.SMOOTH.getWidth(s) : mc.textRenderer.getWidth(s);
	}
	public static float getCustomHeight() {
		return (isCustomFont() ? FontRenderers.SMOOTH.getFontHeight() : mc.textRenderer.fontHeight) * 2;
	}
	...
	public static void drawCustomText(DrawContext drawContext, String text, double x, double y, int color) {
		...
		if (true) {
			...
			FontRenderers.SMOOTH.drawString(drawContext.getMatrices(), text, (float) x, (float) y + 2, color);
		} else {
			drawContext.drawText(mc.textRenderer, text, (int) x, (int) y, color, true);
		}
		matrixStack.pop();
	...
	public static void drawCustomSmallText(DrawContext drawContext, String text, double x, double y, int color) {
		if (true) {
			FontRenderers.SMOOTH.drawString(drawContext.getMatrices(), text, (float) x, (float) y + 1, color);
		} else {
			drawContext.drawText(mc.textRenderer, text, (int) x, (int) y, color, true);
		}
	}
	...
	public static void drawText(DrawContext context, String text, Vec3d vector, int color) {
		...
		if (vector.z > 0 && vector.z < 1) {
			...
			float scale = (float) Math.max(1 - mc.player.getEyePos().distanceTo(preVec) * 0.025, 0);
...
//		char[] stringToCharArray = Chat.chatMessage.get(text).getString().toCharArray();
//		int index = 0;
//		boolean color = false;
//		String s = null;
